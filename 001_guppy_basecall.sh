...
echo ' '
export i=${1} # 1, 2, ... 8
export D_IN=${FAST5_FILE_DIRECTORY} # directory where input fast5 files are stored.
...
time cp -pR ${D_IN}/*/${IN_FAST5} ${TMPDIR}/input
echo ' '
...
if [ -e ${D_OUT}/${ID}/pass/${BARCODE}/*.fastq.gz ] ; then
  ...
  mv ${D_OUT}/${ID2}/pass/${BARCODE}/*.fastq.gz \
  ${D_OUT}/fastq_pass/${BARCODE}/FAM97239_pass_${BARCODE}_${ID2}.fastq.gz ; 
fi ;
done
echo ' '
