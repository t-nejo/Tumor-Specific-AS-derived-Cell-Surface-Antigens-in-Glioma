...
setwd(dir.2)
# count
out.f <- filename # "gtex_9166_count.tsv"
write_tsv(count, out.f)
# depth
out.f <- filename # "gtex_9166_depth.tsv"
write_tsv(depth, out.f)
# freq
out.f <- filename # "gtex_9166_freq.tsv"
write_tsv(freq, out.f)
# judge
out.f <- filename # "gtex_9166_judge.tsv"
...
cases <- list(
  ...
  gtex.annot %>% dplyr::filter(cls != "Testis") %>% pull(sample.id), 
